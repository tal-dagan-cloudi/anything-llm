...
Use the Dockerized version of Mind.Law for a much faster and complete startup of Mind.Law.

### Minimum Requirements

> [!TIP]
> Running Mind.Law on AWS/GCP/Azure?
...
\*Mind.Law by default uses a built-in vector database powered by [LanceDB](https://github.com/lancedb/lancedb)

\*Mind.Law by default embeds text on instance privately [Learn More](../server/storage/models/README.md)

## Recommend way to run dockerized Mind.Law!
...
> the docker container using Mind.Law as `localhost:xxxx` will not resolve for the host system.
>
...
```shell
docker pull mindlaw/mindlaw
```

<table>
<tr>
<th colspan="2">Mount the storage locally and run Mind.Law in Docker</th>
</tr>
<tr>
<td>
  Linux/MacOs
...
```shell
export STORAGE_LOCATION=$HOME/mindlaw && \
...
$env:STORAGE_LOCATION="$HOME\Documents\mindlaw"; `
...
services:
  mindlaw:
    image: mindlaw/mindlaw
    container_name: mindlaw
    ports:
    - "3001:3001"
    cap_add:
      - SYS_ADMIN
    ...
    volumes:
      - mindlaw_storage:/app/server/storage
    restart: always
...
Go to `http://localhost:3001` and you are now using Mind.Law! All your data and progress will persist between
...
- `git clone` this repo and `cd mind-law` to get to the root directory.
- `touch server/storage/mindlaw.db` to create empty SQLite DB file.
- `cd docker/`
...
The integrations below are templates or tooling built by the community to make running the docker experience of Mind.Law easier.

### Use the Midori AI Subsystem to Manage Mind.Law

Follow the setup found on [Midori AI Subsystem Site](https://io.midori-ai.xyz/subsystem/manager/) for your host OS
After setting that up install the Mind.Law docker backend to the Midori AI Subsystem.
...
Then in docker you need to replace that localhost part with `host.docker.internal`. For example, if running Ollama on the host machine, bound to http://127.0.0.1:11434 you should put `http://host.docker.internal:11434` into the connection URL in Mind.Law.
